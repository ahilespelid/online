...
if( !function_exists(  'workreap_featured_freelancer_tag' ) ) {
	function workreap_featured_freelancer_tag($user_id='',$print_class='no'){
		...
		$get_expiry		= get_post_meta($linked_profile,'_expiry_string',true);
									...
									if( !empty( $view ) && $total_cats > $view ) {
										...
										if( $count_cats == $view ) {
											if( $total_cats >= $view){
												...
												echo '<li class="showmore_skills" data-id="'.esc_attr( $post_id ).'-'.esc_attr($uniq_flag).'"><a class="wt-moretags" href="#" onclick="event_preventDefault(event);">'.sprintf(esc_html('+%s more','workreap'),$more_count).'</a></li>';;
											}
										}
									}
									$count_cats++;
...
if (!function_exists('workreap_profile_strength_html')) {
	...
	function workreap_profile_strength_html($user_id='',$show=false){
		...
		$titles	= array(
            ...
            'description' 	=> esc_html__('Description', 'workreap'),
							...
							jQuery(document).on('ready', function () {
								jQuery.ajax({
									...
									data: {
										...
										id		: <?php echo intval($user_identity);?>,
